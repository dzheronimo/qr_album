...
export function PricingTable({
  ...
  const [currency, setCurrency] = useState<Currency>(initialCurrency || 'RUB');
  ...
  return (
    <div className={className}>
      <div className="text-center mb-8">
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <div className="flex bg-muted rounded-lg p-1">
            <Button variant={currency === 'RUB' ? 'default' : 'ghost'} size="sm" onClick={() => handleCurrencyChange('RUB')}>
              ₽ RUB
            </Button>
            <Button variant={currency === 'EUR' ? 'default' : 'ghost'} size="sm" onClick={() => handleCurrencyChange('EUR')}>
              € EUR
            </Button>
          </div>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm text-yellow-800">Не удалось загрузить актуальные тарифы. Показаны ориентировочные цены.</p>
              </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {plans.map((plan) => (
          <PlanCard key={plan.id} plan={plan} currency={currency} onSelect={onPlanSelect} />
        ))}
      </div>
